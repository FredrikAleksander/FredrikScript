...
module Parser =
    ...
    let private pClass : Parser<PTypeDeclaration> =
        let pBody = 
            pKeywordBraceStart >>. spaces >>. many pTypeMember .>> spaces.>> pKeywordBraceEnd .>> spaces
        pKeywordClass >>. spaces1 >>. pClassName .>>. pContextInfo .>> spaces .>>. opt (pInheritance .>> spaces) .>>. pBody 
        |>> (fun (((n,pos),t),m) -> 
            match t with
            | Some t' -> PClass(pos, Name n,t',m)
            | _ -> PClass(pos, Name n, [], m))
        <?> "struct"
    let pType =
